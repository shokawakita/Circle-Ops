...
| `place` | string | `"渋谷駅徒歩15分"` | 短文 | 簡易場所表示 | ✅ | ⚠️ 未検証（`event[place]` と推測） |
| `place_detail` | string | `"氷川区民会館 集会室"` | 短文 | 会場詳細名 | ✅ | ⚠️ 未検証（`event[place_detail]`） |
...
| `pref_id` | integer | `13` | 1〜2桁 | 都道府県ID。JIS X 0401の都道府県コードと推測（13=東京都は一致）。他県は未検証 | ✅ | ⚠️ 未検証（`event[pref_id]`）。対応表は `client.py` の `PREFECTURE_IDS` |
| `capacity` | integer | `20` | — | イベント全体の定員。**プラン定員の合計と一致しなくてよい**（早割系が埋まって閉じる設計のため） | ✅ | ⚠️ 未検証（`event[capacity]`） |
| `min_num_of_people` | integer | `4` | — | 最小催行人数 | ✅ | ⚠️ 未検証（`event[min_num_of_people]`） |
| `application_due_date` | string (datetime) | `"2026-09-23T12:55:00.000+09:00"` | ISO8601。実例は開始5分前 | 申込締切日時 | ✅ | ⚠️ 未検証（`event[application_due_date]`） |
| `event_end_datetime` | string (datetime) | `"2026-09-23T17:00:00.000+09:00"` | ISO8601 | 終了日時 | ✅ | ⚠️ 未検証（`event[event_end_datetime]`） |
...
`events_plans[]` として送るプラン配列。GETでの各要素のキー名は、書き込み側とは異なる（`plan`/`creator_price` vs `name`/`price`）。

| フィールド（GET） | 型 | 観測値の例 | 意味 | Read | Write（対応する送信キー） |
|---|---|---|---|---|---|
| `id` | integer | `1135434` | プランID | ✅ | — サーバー生成 |
| `plan` | string | `"【⚠️⚠️進行役専用⚠️⚠️】参加チケット"` | プラン名。`【】`や絵文字を含めてよい | ✅ | ✅ 検証済み（`events_plans[][name]`） |
| `capacity` | integer | `5` | そのプランの定員 | ✅ | ✅ 検証済み（`events_plans[][capacity]`） |
| `creator_price` | integer | `500` | 価格（円） | ✅ | ✅ 検証済み（`events_plans[][price]`。**GETとPOSTでキー名が違う点に注意**） |
